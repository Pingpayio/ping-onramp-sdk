...
const OnrampStepContent = ({
    ...
    switch (currentStep) {
      ...
      case 1:
        return <div className="flex flex-col h-full">
            ...
            {/* Transaction progress container with full height to match asset selection */}
  ...
  return <div className="flex flex-col h-full">
      {/* Content section with consistent appearance */}
      <div className="flex-grow overflow-auto pb-1">
        {renderCurrentStep()}
      </div>
      
      {/* Navigation buttons with consistent styling - only shown in step 0 */}
      {currentStep === 0 && (
        <div className="mt-auto pt-4">
          <OnrampNavigation currentStep={currentStep} steps={steps} handleBack={handleBack} handleContinue={handleContinue} canContinue={canContinue} isProcessingTransaction={isProcessingTransaction} />
        </div>
      )}
    </div>;
};
