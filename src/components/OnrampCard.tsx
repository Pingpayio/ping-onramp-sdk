  ...
  return (
    <div 
      className={cn(
        ...
        isSelected ? "border-2 border-ping-600 shadow-lg ping-shadow" : "border-border hover:shadow-md"
      )}
      onClick={onClick}
    >
        ...
        <div className="flex items-center">
          <div className="bg-ping-100 p-2 rounded-full mr-3">
            {icon === 'card' ? (
              ...
              <Wallet className="h-6 w-6 text-ping-600" />
            )}
          </div>
          <h3 className="font-semibold text-xl">{title}</h3>
        </div>
        {isSelected && (
          <div className="bg-ping-600 h-6 w-6 rounded-full flex items-center justify-center">
      ...
      <div className="mt-auto text-sm font-medium text-ping-600 z-10">Powered by {provider}</div>
    </div>
  );
};
