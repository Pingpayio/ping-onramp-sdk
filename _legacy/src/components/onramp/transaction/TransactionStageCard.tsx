...
import { CheckCircle2, CircleX, Clock, ArrowRight, Wallet, Link, Loader2, SearchCheck } from 'lucide-react';
...
export const TransactionStageCard: React.FC<TransactionStageCardProps> = ({
  ...
  const getStageDescription = () => {
    ...
    if (displayInfo?.message && intentProgress !== 'error') return displayInfo.message;

    const assetDisplay = asset || 'asset';
    const amountInDisplay = displayInfo?.amountIn?.toFixed(2) || amount || 'specified amount';
    ...
    switch (intentProgress) {
      case 'none': return 'Initializing onramp process...';
      case 'form': return 'Please fill out the form to proceed.';
      case 'generating_url': return 'Preparing your secure onramp link...';
      case 'redirecting_coinbase': return 'Redirecting you to Coinbase to complete your purchase.';
      case 'depositing': return `Waiting for your ${amountInDisplay} ${selectedCurrencySymbol(assetDisplay)} deposit from Coinbase.`;
      case 'querying': return `Finding the best bridge rates for your ${assetDisplay}.`;
      case 'signing': return `Please sign the transaction in your wallet to bridge ${assetDisplay} to ${recipientDisplay}.`;
      case 'withdrawing': return `Processing your ${assetDisplay} bridge to NEAR. This may take a few moments.`;
      case 'done': {
        const outAmount = displayInfo?.amountOut ? `${displayInfo.amountOut.toFixed(Math.min(6, assetDecimalPlaces(assetDisplay)))} ${assetDisplay}` : `${assetDisplay}`;
        return `Successfully sent ${outAmount} to ${recipientDisplay}.`;
      }
      case 'error': return error || displayInfo?.error || 'The transaction could not be completed.';
      default: return 'Processing your transaction...';
    }
  };
  ...
  const renderTransactionHash = () => {
    ...
    if (txHashToDisplay) {
      const isFullUrl = txHashToDisplay.startsWith('http://') || txHashToDisplay.startsWith('https://');
      const url = isFullUrl ? txHashToDisplay : (explorerBaseUrl + txHashToDisplay);
