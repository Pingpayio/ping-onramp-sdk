...
export function getChainConfig(chainName: string): ChainConfig {
    ...
    switch (lowerChainName) {
        case 'base':
            ...
            if (!process.env.BASE_RPC_URL) throw new Error("BASE_RPC_URL environment variable not set.");
            return { viemChain: base, rpcUrl: process.env.BASE_RPC_URL };
        case 'ethereum': // or 'eth'
            if (!process.env.ETHEREUM_RPC_URL) throw new Error("ETHEREUM_RPC_URL environment variable not set.");
            return { viemChain: mainnet, rpcUrl: process.env.ETHEREUM_RPC_URL };
        case 'arbitrum':
            if (!process.env.ARBITRUM_RPC_URL) throw new Error("ARBITRUM_RPC_URL environment variable not set.");
            return { viemChain: arbitrum, rpcUrl: process.env.ARBITRUM_RPC_URL };
        case 'polygon':
            if (!process.env.POLYGON_RPC_URL) throw new Error("POLYGON_RPC_URL environment variable not set.");
            return { viemChain: polygon, rpcUrl: process.env.POLYGON_RPC_URL };
        // Add other chains your application supports for deposits
        default:
            throw new Error(`Unsupported chain for balance check: ${chainName}. Please configure its RPC URL and viem chain object in getChainConfig.`);
    }
}
// --- End Helper ---
...
export async function getCurrentTokenBalance(
    ...
    try {
        ...
        console.error(`Error fetching balance for ${tokenContractAddress} on ${chainName} for ${userAddress}:`, error);
...
export async function pollForDepositConfirmation(
    ...
    console.log(`Polling for deposit: Addr=${userAddress}, Token=${tokenContractAddress}, Chain=${chainName}, InitialBalance=${initialBalance}, ExpectedIncrease=${expectedMinimumIncrease}`);

    while (Date.now() - startTime < timeout) {
        attempts++;
        try {
            const currentBalance = await getCurrentTokenBalance(userAddress, tokenContractAddress, chainName);
            if (options?.onPoll) {
                options.onPoll(currentBalance, attempts);
            }

            if (currentBalance >= initialBalance + expectedMinimumIncrease) {
                console.log(`Deposit confirmed for ${userAddress}. New balance: ${currentBalance}. Increase: ${currentBalance - initialBalance}`);
                return currentBalance;
            }
        } catch (error) {
            // Log error from getCurrentTokenBalance but continue polling unless it's a fatal config error
            console.warn(`Attempt ${attempts}: Error polling for balance (will retry):`, (error as Error).message);
            if ((error as Error).message.includes("Unsupported chain") || (error as Error).message.includes("environment variable not set")) {
    ...
    throw new Error(`Timeout waiting for deposit confirmation for ${userAddress} on ${tokenContractAddress}. Expected increase of ${expectedMinimumIncrease} over initial ${initialBalance} not observed.`);
}
