...
import { useNearIntentsDisplayInfo, useOneClickStatus } from "../../state/hooks"; // Added hooks
...
const getDisplayInfoForStatus = (
...
): { progress: number; box: StepBox; detailsForDisplay: Record<string, string | undefined> } => {
  ...
  if (oneClickStatus) {
    detailsForDisplay.amountIn = oneClickStatus.quoteResponse.quote.amountInFormatted;
    detailsForDisplay.amountOut = oneClickStatus.quoteResponse.quote.amountOutFormatted;
    ...
    switch (oneClickStatus.status) {
      ...
      case "SUCCESS":
        ...
        detailsForDisplay.explorerLink = oneClickStatus.swapDetails?.destinationChainTxHashes?.[0]?.explorerUrl || nearIntentsDisplayInfo?.explorerUrl;
        break;
      case "REFUNDED":
      case "FAILED":
      case "EXPIRED":
      ...
      default:
        ...
        box.desc = nearIntentsDisplayInfo?.message || "Waiting for status updates...";
    }
...
export function ProcessingOnramp({
  ...
  if (step === 3 && result?.success) { // "complete" step
    displayProgress = 100;
    displayBox = {
      icon: <FaCheckCircle className="text-green-400 text-xl" />,
      title: "Transaction Complete",
  ...
  } else if (step === 0) { // "initiating-onramp-service"
    displayProgress = 15;
     displayBox = {
      icon: <FaSpinner className="animate-spin text-blue-400 text-xl" />,
      title: "Initiating Onramp",
      desc: nearIntentsDisplayInfo.message || "Preparing your onramp with Coinbase...",
      color: "border-blue-400",
    };
    transactionDetails.statusMessage = nearIntentsDisplayInfo.message;
  } else if (step === 2) { // "processing-transaction" - main 1Click polling phase
    const statusDerivedInfo = getDisplayInfoForStatus(oneClickStatus, nearIntentsDisplayInfo);
    displayProgress = statusDerivedInfo.progress;
    displayBox = statusDerivedInfo.box;
    transactionDetails = { ...transactionDetails, ...statusDerivedInfo.detailsForDisplay };
  } else { // Default/fallback or other steps
     displayBox = {
        ...
        <div className="flex flex-col gap-1 text-sm">
          {transactionDetails.statusMessage && !oneClickStatus?.status && (
             <div className="flex justify-between">
              ...
              <span className="text-gray-100">{transactionDetails.statusMessage}</span>
            </div>
          )}
          {transactionDetails.originAsset && (
            <div className="flex justify-between">
              <span className="text-gray-400">From Asset ID:</span>
              <span className="text-gray-100 truncate max-w-[180px]">{transactionDetails.originAsset}</span>
            </div>
          )}
          {transactionDetails.amountIn && (
            <div className="flex justify-between">
              <span className="text-gray-400">Amount In:</span>
              <span className="text-gray-100">{transactionDetails.amountIn}</span>
            </div>
          )}
          {transactionDetails.destinationAsset && (
            <div className="flex justify-between">
              <span className="text-gray-400">To Asset ID:</span>
              <span className="text-gray-100 truncate max-w-[180px]">{transactionDetails.destinationAsset}</span>
            </div>
          )}
          ...
          {transactionDetails.recipient && (
            <div className="flex justify-between">
              ...
              <span className="text-gray-100 truncate max-w-[180px]">{transactionDetails.recipient}</span>
            </div>
          )}
           {transactionDetails.transactionId && (
            <div className="flex justify-between">
              <span className="text-gray-400">Transaction ID:</span>
              <span className="text-gray-100 truncate max-w-[180px]">{transactionDetails.transactionId}</span>
            </div>
          )}
           {transactionDetails.service && (
            <div className="flex justify-between">
              <span className="text-gray-400">Service:</span>
              <span className="text-gray-100">{transactionDetails.service}</span>
            </div>
          )}
        </div>
      </div>
